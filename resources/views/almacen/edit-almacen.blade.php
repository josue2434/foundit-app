<x-app-layout>
    <div class="min-h-screen flex items-center justify-center p-6">
        <!-- Formulario de Edición de Almacén -->
        <div class="max-w-4xl w-full bg-white rounded-lg shadow-lg border border-gray-200 relative z-10">
            {{-- Los datos del almacén se pasan desde la ruta en web.php --}}
            {{-- El action="#" y method="POST" son solo para la estructura visual del formulario, no hay funcionalidad de backend --}}
            <form action="#" method="POST" class="p-8">
                @csrf
                @method('PUT') {{-- Se mantiene para la estructura visual --}}
                <div class="space-y-6">
                    <!-- Header -->
                    <div class="text-center">
                        <h1 class="text-2xl font-semibold text-[#2045c2]">EDITAR ALMACÉN</h1>
                        <p class="text-gray-600 mt-1">Modifique los detalles del almacén</p>
                    </div>
                    <!-- Nombre del Almacén -->
                    <div>
                        ...
                        <input
                            ...
                            required
                            title="Modifique el nombre completo del almacén"
                        >
                        @error('nombre')
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="w-full h-12 px-4 text-lg rounded-lg border border-gray-300 focus:border-[#2045c2] focus:ring-2 focus:ring-[#2045c2] focus:ring-opacity-20 transition-all duration-200"
                            required
                            title="Modifique la dirección completa del almacén"
                        >
                        @error('direccion')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        >
                            ...
                            @for ($i = 1; $i <= 20; $i++)
                                <option value="{{ $i }}" {{ old('cantidad_estantes', $almacen['cantidad_estantes'] ?? '') == $i ? 'selected' : '' }}>{{ $i }} Estantes</option>
                            @endfor
                        </select>
                ...
                <div class="flex justify-end space-x-4 mt-8 pt-6 border-t border-gray-200">
                    <a
                        ...
                        title="Cancelar la edición y volver a la lista de almacenes"
                    >
                    ...
                    <button
                        ...
                        title="Guardar los cambios realizados al almacén"
                        id="submit-btn"
                    >
